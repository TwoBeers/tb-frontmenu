...
tb_frontmenu = {
	...
	init : function() {
		...
		$( '.top-item-container', menu ).hoverIntent(
			...
			function() { //when mouse leaves, hide the sub list
				...
				$( '.sub-menu', parent ).stop().removeClass( 'expanded not-extended extended' ).slideUp();

				$( '.sub-menu-toggler', parent ).removeClass( 'dashicons-arrow-up-alt2' ).addClass( 'dashicons-arrow-down-alt2' );

				$( '.attachment-tb-frontpage-thumb', parent ).stop().css( 'opacity', 1 );
		...
		$( '.top-item-container > a', menu ).hoverIntent(
			...
			function(){ //when mouse enters, slide down the sub list
				...
				$( '.sub-menu:not(.expanded)', parent ).css( 'height', 'auto' );
				...
				$( '.sub-menu:not(.expanded)', parent ).css( 'height', to_height ).slideDown( 'slow', 'easeInOutCubic' ).addClass( 'expanded ' + ext_class );

				if ( parent.hasClass( 'menu-item-has-children' ) )
					$( '.attachment-tb-frontpage-thumb', parent ).animate({ opacity: 0 }, "slow" );
	...
	resizeStuff: function() {
		...
		if ( tb_frontmenu_data.test_mode == 1 ) {
			...
			$( '#tb-frontmenu-windowsize' ).html( 'current window width: ' + $( window ).width() + 'px' );
